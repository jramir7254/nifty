...
// Detect development mode (Next.js compatible; safe fallback)
const isDev: boolean =
...
const methods = {
    debug: (...args: unknown[]) => {
        if (!isDev) return;
        console.debug(...(buildArgs('debug', args) as unknown[]));
    },
    info: (...args: unknown[]) => {
        if (!isDev) return;
        console.info(...(buildArgs('info', args) as unknown[]));
    },
    success: (...args: unknown[]) => {
        if (!isDev) return;
        console.log(...(buildArgs('success', args) as unknown[]));
    },
    warn: (...args: unknown[]) => {
        if (!isDev) return;
        console.warn(...(buildArgs('warn', args) as unknown[]));
    },
    error: (err: unknown, ...args: unknown[]) => {
        ...
        if (err instanceof Error) {
            console.error(...(buildArgs('error', [err.message, ...args]) as unknown[]));
        ...
        console.error(...(buildArgs('error', [err, ...args]) as unknown[]));
    },
    fatal: (...args: unknown[]) => {
        if (!isDev) return;
        console.error(...(buildArgs('fatal', args) as unknown[]));
    },
    trace: (...args: unknown[]) => {
        if (!isDev) return;
        console.trace(...(buildArgs('trace', args) as unknown[]));
    },
    ...
    json: (obj: unknown) => {
        if (!isDev) return;
        console.log(...(buildArgs('info', [JSON.stringify(obj, null, 2)]) as unknown[]));
    },
...
function create(namespace: string): Logger {
    ...
    const namespaced = Object.fromEntries(
        Object.entries(methods).map(([name, fn]) => [name, wrap(fn as unknownFn)])
